...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        // Verify password using password_verify() (ensure password is hashed during registration)
        if (password_verify($password, $row['password'])) {
            $_SESSION['loggedin'] = true;  // Set session variable
            $_SESSION['name'] = $row['name'];  // Save user's name to the session
...
<?php include('inc/container.php');?>
	...
	<h1 class="text-center my-4 py-3 text-light" id="title">Restaurant Inventory Management System</h1>	
